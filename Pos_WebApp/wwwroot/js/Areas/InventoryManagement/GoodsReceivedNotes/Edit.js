...
function getSubItemRow(rowNo, model, isLastCheck) {
    if (model) {
        model.Id = rowNo;
    }
    else {
...
$(document).ready(function () {
    ...
    //default (today) date for grn-date.
    if ($("#GrnDate").val() == "") {
    ...
    $('#edit-grn-form').submit(function (e) {
        e.preventDefault();
        FormIsValid = true;
        NonEmptyRowsCount = 0;
        ...
        if (FormIsValid && NonEmptyRowsCount !== 0) {
            var submitBtnId = 'btn-edit-grn';
            var submitToUrl = '/GoodsReceivedNotes/Edit';
            var formId = 'edit-grn-form';
