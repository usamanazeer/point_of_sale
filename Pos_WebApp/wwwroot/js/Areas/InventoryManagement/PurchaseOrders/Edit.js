...
$(document).ready(function () {
    ...
    //default (today) date for po-date.
    if ($("#PoDate").val() == "") {
    ...
    for (var i = 0; i < rowsCount; i++) {
        if ($("#InvPoDetails_" + i + "__Rate").val() === "0" || $("#InvPoDetails_" + i + "__Rate").val() == "") {
            var Rate = $("#InvPoDetails_" + i + "__ItemId option:selected")[0].getAttribute('data-purchaseRate');
            $("#InvPoDetails_" + i + "__Rate").val(Rate);
        }
        CalculatePoDetails_Total(i);
    }

    $('#edit-po-form').submit(function (e) {
        e.preventDefault();
        FormIsValid = true;
        NonEmptyRowsCount = 0;
        ...
        if (FormIsValid && NonEmptyRowsCount !== 0) {
            var submitBtnId = 'btn-edit-po';
            var submitToUrl = '/PurchaseOrders/Edit';
            var formId = 'edit-po-form';
