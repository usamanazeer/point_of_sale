bindSubmit('edit-item-form', 'btn-edit-item', '/Items/Edit', null, null, function (res) {
    if (res.responseCode === CODES.RESPONSE_CODE_UPDATED) {
        if ($('#item-image-input').val() !== "") {
            $(`#item-image`).attr('src', res.model.imageUrl);
        }
    }
});
...
$(document).ready(function () {
    ...
    if ($("#IsRecipe")[0].checked || $("#IsDeal")[0].checked) {
        ...
        for (var i = 0; i < rowsCount; i++) {
            var SalesRate = $("#InvItemRecipeChild_" + i + "__ItemId option:selected")[0].getAttribute('data-purchaseRate');
            var Measurement = $("#InvItemRecipeChild_" + i + "__ItemId option:selected")[0].getAttribute('data-itemMeasurement');
            var Unit = $("#InvItemRecipeChild_" + i + "__ItemId option:selected")[0].getAttribute('data-itemUnit');
    ...
    if ($("#addMiodifiersSwitch")[0].checked) {
        IsModifiersTableInited = true;

        //calculate modifiers totals etc
        var rowsCount = $("table#modifiers-table tbody tr").length;
        for (var i = 0; i < rowsCount; i++) {
            var modifiersCharges = $("#InvItemModifiers_" + i + "__ModifierId option:selected")[0].getAttribute('data-ModifierCharges');
...
function MainCategory_OnChange() {
    ...
    if (categoryId != "") {
        $.ajax({
            url: '/SubCategory/Get/?parentCategoryId=' + categoryId,
            type: 'GET',
            success: function (res) {
                //SUCCESS
                if (res.subCategories != null) {
                    for (var i in res.subCategories) {
                        var subCategory = res.subCategories[i];
            ...
            error: function () {
                sweetAlert.error({ text: 'An error occcured while updating role.' });
            },
        });
    }
    else {
...
//    //    console.log('tax is in percent? ', taxInPercent);
//    //}
...
//    //}
//    $("#FinalSalesRate").val(Number(result.toFixed(2)));
//}
...
function IsRawItem_OnClick() {
    ...
    if ($("#IsRawItem")[0].checked) {
        ...
        //for modifiers
        $('#addMiodifiersSwitch')[0].checked = false;
        $('#addMiodifiersSwitch').attr('disabled', 'disabled');
        $('#modifiers-table-container').attr('hidden', 'hidden');
        $('#modifiers-table tbody').html('<tr></tr>');
        $('#modifiers-deleted-rows-table').html('<tr></tr>');
    }
    else {
        $('#addMiodifiersSwitch').removeAttr('disabled');
...
function InitSubItemsTable() {
    ...
    $('table#deleted-rows-table tbody').html('<tr></tr>');
    getSubItemRow(0);
...
function getSubItemRow(rowNo) {
    ...
    $.ajax({
        url: '/Items/GetAddItemRecipeRow?rowNo=' + rowNo,
        type: 'GET',
...
function ItemBarCodeSelect_OnChange(rowNo) {
    if (flag) {
        var ItemId = $("#InvItemRecipeChild_" + rowNo + "__BarCodeId option:selected")[0].getAttribute('data-ItemId');
...
function ItemSelect_OnChange(rowNo) {
    var SalesRate = $("#InvItemRecipeChild_" + rowNo + "__ItemId option:selected")[0].getAttribute('data-purchaseRate');
    var Measurement = $("#InvItemRecipeChild_" + rowNo + "__ItemId option:selected")[0].getAttribute('data-itemMeasurement');
    ...
    if (flag) {
        var ItemBarCode = $("#InvItemRecipeChild_" + rowNo + "__ItemId option:selected")[0].getAttribute('data-ItemBrCode');
        ...
        flag = false;
        if (ItemBarCode != "") {
            var ItemBarCodeId = $("#InvItemRecipeChild_" + rowNo + "__BarCodeId option:contains(" + ItemBarCode + ")").val();
...
function ModifierSelect_OnChange(rowNo) {
    var ModifierCharges = $("#InvItemModifiers_" + rowNo + "__ModifierId option:selected")[0].getAttribute('data-ModifierCharges');
